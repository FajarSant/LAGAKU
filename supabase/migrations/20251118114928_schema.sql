...
-- =======================================================
-- TABLE: pengguna (SINKRON DENGAN auth.users)
-- =======================================================
...
create table if not exists public.acara (
  ...
  cabang_olahraga_id uuid
      references public.cabang_olahraga(id)
      on delete set null,

  lokasi text,

  dibuat_oleh uuid
      references public.pengguna(id)
      on delete set null,
...
create index if not exists anggota_tim_peserta_idx on public.anggota_tim(peserta_id);
...
create table if not exists public.pertandingan (
  ...
  peserta_tuan_rumah_id uuid
      references public.peserta(id)
      on delete set null,

  peserta_tamu_id uuid
      references public.peserta(id)
      on delete set null,
